  ...
  `(defmethod ,name ((op ,op) (component ,component))
     (let ((output-files ()))
       (do-components (child component)
         (dolist (out (,name op child))
           (push out output-files)))
       output-files)))

(define-downward-appending asdf:input-files preprocess-op c-system)
(define-downward-appending asdf:input-files assemble-op c-system)
(define-downward-appending asdf:input-files link-op c-system)
(define-downward-appending asdf:output-files preprocess-op c-system)
(define-downward-appending asdf:output-files assemble-op c-system)
